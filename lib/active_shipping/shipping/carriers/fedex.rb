  ...
  module Shipping
    ...
    class FedEx < Carrier
      ...
      def build_rate_request(origin, destination, packages, options={})
        ...
        xml_request = XmlNode.new('RateRequest', 'xmlns' => 'http://fedex.com/ws/rate/v8') do |root_node|
          root_node << build_request_header

          # Version
          root_node << XmlNode.new('Version') do |version_node|
            version_node << XmlNode.new('ServiceId', 'crs')
            version_node << XmlNode.new('Major', '8')
      ...
      def build_location_node(name, location)
        location_node = XmlNode.new(name) do |xml_node|
          xml_node << XmlNode.new('Address') do |address_node|
            address_node << XmlNode.new("StreetLines", location.address1) unless location.address1.blank?
            address_node << XmlNode.new("StreetLines", location.address2) unless location.address2.blank?
            address_node << XmlNode.new("StreetLines", location.address3) unless location.address3.blank?
            ...
            address_node << XmlNode.new("Residential", location.residential?)
          end
        end
      end
      ...
      def parse_rate_response(origin, destination, packages, response, options)
        ...
        root_node.elements.each('RateReplyDetails') do |rated_reply_details|
          ...
          rate_estimates << RateEstimate.new(origin, destination, @@name,
                              ...
                              :transit_time            => rated_reply_details.get_text('TransitTime').to_s)
	      end
